...
| 変更 | 主な利点 | 主な注意点 |
|---|---|---|
| `g_m1` を大きくする | GBWとユニティゲイン周波数を上げやすい | `fu` が非支配極へ近づくと位相余裕が低下する。電流と寄生容量も増えやすい |
| `g_m6` を大きくする | 非支配極とRHP零点を高周波側へ移動しやすい | 第2段のバイアス電流と消費電力が増える |
| `C_C` を大きくする | 支配極を形成しやすい | GBW、`fu`、スルーレートが低下し、RHP零点も低周波側へ移動する |
| `C_L` を小さくする | 非支配極を高周波側へ移動し、位相余裕を確保しやすい | 外部回路によって決まり、自由に変更できない場合がある |
| `I5` を大きくする | スルーレートを上げやすい | 消費電力が増え、動作点や小信号パラメータも変化する |
| 位相余裕 `PM` を大きくする | オーバーシュートとリンギングを抑えやすい | 補償を強くしすぎると帯域幅と応答速度が低下する |
| 帰還係数 `β` を小さくする | 閉ループ利得を高くでき、0 dB交差周波数が低下する | 閉ループ利得、帯域幅、誤差の関係を確認する必要がある |

一つの性能だけを最大にすればよいわけではない。低周波利得、GBW、位相余裕、スルーレート、出力負荷、消費電力のバランスを取る必要がある。
