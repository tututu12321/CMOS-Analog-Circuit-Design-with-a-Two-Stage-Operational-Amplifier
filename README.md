...
| 記号 | 意味 |
|---|---|
| A_v | 出力電圧の変化を入力電圧の変化で割った値 |
| R_out | 出力ノードから見た合成抵抗 |
| - | 入力と出力が反対方向に変化することを表す |

抵抗負荷R_DとMOSFETの出力抵抗r_oを含める場合は、次のようになる。
...
MOSFETを増幅に使うには、飽和領域で動作させる必要がある。

**結論**

ソース接地増幅回路は、入力信号を増幅し、向きを反対にして出力する。
...
MOSFETで作る負荷を能動負荷という。電流源負荷は能動負荷の代表的な使い方である。

| 負荷 | 特徴 |
|---|---|
| 抵抗負荷 | 動作は分かりやすいが、大きな抵抗は面積を使う |
| 能動負荷 | 小さな面積で大きな出力抵抗を作りやすい |
| 電流源負荷 | バイアス電流を決めながら能動負荷として働く |

NMOS増幅素子とPMOS能動負荷が同じ出力ノードに接続される場合、出力抵抗は次のようになる。
...
| parallel | 2つの抵抗を並列に接続した値 |

出力抵抗が大きいほど、同じ電流変化から大きな出力電圧を作れる。

~~~text
abs(A_v) ≈ g_m × R_out
~~~

ただし、NMOSとPMOSを飽和領域に保つための電圧が必要である。出力電圧が電源電圧へ近づきすぎると、どちらかが線形領域へ入り、利得が低下する。

**結論**

能動負荷は、大きな出力抵抗を作り、増幅回路の電圧利得を高くする。
...
| 種類 | 電流の役割 | 飽和領域の条件 |
|---|---|---|
| NMOS電流ミラー | 出力ノードからGND側へ電流を引く | V_DS ≥ V_OV |
| PMOS電流ミラー | 電源側から出力ノードへ電流を供給する | V_SD ≥ V_OVp |

実際には、MOSFETのばらつき、チャネル長変調、基準側と出力側の電圧差によって誤差が生じる。
...
MOS差動対は、2個の入力MOSFETのソースを接続し、その共通部分へ電流源を接続した回路である。

2つの入力電圧の差を差動入力電圧v_idとする。
...
入力MOSFETをM_1とM_2、各ドレイン電流をI_D1とI_D2、共通の電流源をI_TAILとする。
...
2つの入力電圧が等しく、M_1とM_2の特性も等しい場合、電流は半分ずつ流れる。

~~~text
I_D1 ≈ I_D2 ≈ I_TAIL / 2
~~~

v_in+が高くなると、M_1の電流が増え、M_2の電流が減る。

小さな差動入力に対する各枝の電流変化は、次のようになる。
...
この式は、2個の入力MOSFETが同じ特性を持ち、v_idが十分に小さい場合に使える。
...
テール電流I_TAILは、差動対の2本の枝へ流れる電流の合計を決める。
...
差動入力が大きくなると、I_TAILのほとんどが片方のMOSFETへ流れる。このため、差動対が直線的に増幅できる入力範囲には限界がある。

平衡状態における各MOSFETの動作電流は、次のようになる。

~~~text
I_D ≈ I_TAIL / 2
~~~

平方則モデルでは、各入力MOSFETのg_mは次のようになる。
...
テール電流源の出力抵抗が大きいほど、2つの入力へ同じ電圧が加わったときにI_TAILが変化しにくくなる。

**結論**

テール電流は、差動対の動作電流、g_m、入力範囲、消費電力に関係する。
...
差動対からは、M_1側とM_2側の2つの電流が出る。

二段構成CMOSオペアンプの第1段では、PMOS電流ミラーを能動負荷として使い、2つの電流変化を1つの出力ノードへまとめる。

1. 差動入力によって一方の電流が増える
2. もう一方の電流は同じ程度だけ減る
3. 電流ミラーが一方の変化を出力側へコピーする
4. 出力ノードで2つの変化が加わる
...
出力電圧の正負は、入力端子と出力側の枝の割り当てによって決まる。

**結論**

電流ミラー能動負荷は、差動対の2つの電流変化をまとめ、単一出力へ変換する。
...
## Overall Operation of a Two-Stage CMOS Operational Amplifier

二段構成CMOSオペアンプは、第1段で2つの入力電圧の差を受け取り、第2段で信号をさらに増幅して出力する。

この章では、次のMOSFET構成を例にして、入力から出力までの動作を順番に確認する。
...
回路図によってMOSFETの番号は変わる。番号ではなく、接続と役割を見て判断することが大切である。
...
第1段は、NMOS差動対M1、M2と、PMOS電流ミラーM3、M4で構成する。M5は、差動対へ流すテール電流を決める。

第1段の出力を中間ノードv_1とする。v_1は、第2段のMOSFET M6のゲートへ接続される。

第2段は、NMOSのM6とPMOSのM7で構成する。出力は、M6とM7のドレインが接続されたノードv_outから取り出す。

```mermaid
flowchart LR
    A["2つの入力"] --> B["第1段：差動対と電流ミラー"]
    B --> C["中間ノード v_1"]
    C --> D["第2段：ソース接地回路"]
    D --> E["出力 v_out"]
```
...
| 部分 | 主な素子 | 役割 |
|---|---|---|
| 第1段 | M1〜M5 | 入力電圧の差を受け取り、1つの出力電圧へ変える |
| 中間ノード | v_1 | 第1段の出力を第2段のゲートへ伝える |
| 第2段 | M6、M7 | v_1をさらに増幅し、v_outを作る |

実際の回路には、バイアス電圧を作る電流ミラーや、動作を安定させる補償容量C_Cも加えられることが多い。

**結論**

二段構成オペアンプは、第1段の出力電圧v_1で第2段を動かし、最終的な出力電圧v_outを作る。
...
M1とM2はNMOS差動対である。この章では、M1のゲートへv_in-、M2のゲートへv_in+を接続する。

差動入力電圧v_idは、次のように定義する。
...
M1とM2のソースは接続され、その下にテール電流源M5が置かれる。M1とM2のドレイン電流をI_D1、I_D2とすると、合計はほぼ一定になる。

~~~text
I_D1 + I_D2 ≈ I_TAIL
~~~

入力電圧が等しいとき、M1とM2へほぼ同じ電流が流れる。

~~~text
I_D1 ≈ I_D2 ≈ I_TAIL / 2
~~~

v_in+が少し高くなると、M2の電流は増え、M1の電流は減る。v_in-が高くなると、反対の変化が起こる。
...
| 記号 | 意味 |
|---|---|
| i_d1 | M1のドレイン電流の小さな変化 |
| i_d2 | M2のドレイン電流の小さな変化 |
| g_m1 | 入力MOSFETの相互コンダクタンス |

ここではM1とM2が同じ特性を持ち、v_idが十分に小さいものとしている。

差動対だけでは出力が2本あるため、M3とM4の電流ミラーを使って1つの出力v_1へまとめる。

第1段の電圧利得の大きさは、次の式で考えられる。
...
|---|---|
| A_v1 | 第1段の小信号電圧利得 |
| R_out1 | 中間ノードv_1から見た出力抵抗 |

**結論**

第1段は、2つの入力電圧の差を電流差へ変え、電流ミラーを使って1つの中間電圧v_1を作る。

---

## 3.3　電流ミラー能動負荷

### 差動対の2つの電流を1つの出力へまとめる

M3とM4はPMOS電流ミラーであり、M1とM2の能動負荷として働く。

M3はゲートとドレインを接続したダイオード接続である。M3によって決まったゲート電圧がM4にも加わり、M3の電流がM4側へコピーされる。

M3とM4のW / Lが同じ場合は、次の関係になる。
...
中間ノードv_1には、M4が電源側から供給する電流と、M2がGND側へ引く電流の差が現れる。

- M4の供給電流がM2の電流より大きいと、v_1は上がる
- M2の電流がM4の供給電流より大きいと、v_1は下がる

この章の入力接続では、v_in+が高くなるとM2の電流が増え、M1の電流が減る。M1側の電流を写すM4の電流も減るため、v_1は下がる。

中間ノードの出力抵抗は、おおよそ次のようになる。
...
M2とM4の出力抵抗が大きいほど、同じ電流変化から大きなv_1の変化を作れる。

**結論**

M3とM4は、差動対の電流差を単一出力へ変換し、大きな出力抵抗によって第1段の利得を高くする。
...
第2段では、M6をNMOSソース接地増幅回路として使う。M6のゲートへv_1を加え、ドレインからv_outを取り出す。

M7はPMOS電流源負荷であり、電源V_DD側から出力ノードへ電流を供給する。

v_1が変化したときの動作は次のようになる。

| v_1の変化 | M6の電流 | v_outの変化 |
|---|---|---|
| 高くなる | 増える | 低くなる |
| 低くなる | 減る | 高くなる |

第2段は、入力と出力が反対方向に変化する反転増幅段である。

第2段の電圧利得は、おおよそ次のようになる。

~~~text
A_v2 ≈ -g_m6 × R_out2
~~~

負荷抵抗R_Lも含める場合、出力抵抗は次のように考えられる。
...
| r_o7 | M7の小信号出力抵抗 |
| R_L | 出力へ接続される負荷抵抗 |
| R_out2 | 出力ノードv_outから見た合成抵抗 |

M6とM7を飽和領域に保つには、出力電圧に余裕が必要である。単電源回路でソースがV_DDとGNDへ接続されている場合、出力範囲の目安は次のようになる。

~~~text
V_OV6 ≤ v_out ≤ V_DD - V_OV7
~~~
...
この範囲を外れるとM6またはM7が線形領域へ入り、利得が低下しやすい。

**結論**

第2段は、v_1を反転してさらに増幅し、負荷へ渡す出力電圧v_outを作る。
...
バイアス電流は、入力信号がないときにも回路へ流しておく直流電流である。この電流によって、各MOSFETを増幅に使える状態に保つ。

第1段では、M5がテール電流I_TAILを決める。

~~~text
I_D1 + I_D2 ≈ I_TAIL
~~~

入力電圧が等しいときは、I_TAILがほぼ半分ずつ流れる。
...
1. V_DDからM3、M1、M5を通ってGNDへ流れる
2. V_DDからM4、M2、M5を通ってGNDへ流れる

第2段では、M7から供給される電流とM6が引き込む電流が、動作点でほぼ等しくなる。

~~~text
I_D6 ≈ I_D7 ≈ I_BIAS2
~~~

第2段の直流電流は、V_DDからM7、M6を通ってGNDへ流れる。

ここで大切なのは、第1段の電流がそのまま第2段へ流れ込むわけではないことである。第1段はv_1という電圧を作り、その電圧がM6のゲートを動かす。MOSFETのゲートには、直流ではほとんど電流が流れない。

バイアス電流を大きくすると、g_mを大きくして高速に動かしやすい。一方で、消費電力は増える。

単電源回路の直流消費電力は、おおよそ次のように考えられる。
...
|---|---|
| P_DC | 回路が直流で消費する電力 |
| I_SUPPLY | 電源から回路全体へ流れる直流電流 |

**結論**

バイアス電流は、各MOSFETの動作点、増幅能力、速度、消費電力を決める。
...
1. v_in+とv_in-の差がM1、M2へ加わる
2. 差動対が入力電圧の差を2本の電流差へ変える
3. M3、M4の電流ミラーが電流差を1つの出力へまとめる
4. 第1段の出力抵抗によって、電流変化がv_1の変化になる
5. v_1がM6のゲートへ加わる
6. M6のドレイン電流が変化する
7. 第2段の出力抵抗によって、電流変化がv_outの変化になる

この章で決めた入力接続では、信号の向きは次のようになる。
...
したがって、v_in+は非反転入力、v_in-は反転入力として動作する。
...
~~~text
abs(A_v2) ≈ g_m6 × R_out2
~~~

全体の低周波開ループ利得は、2つの利得を掛けた値になる。
...
たとえば、第1段が40倍、第2段が50倍の場合、全体利得は2,000倍になる。

~~~text
abs(A_0) ≈ 40 × 50 = 2,000
~~~

2,000倍をdBで表すと、約66 dBになる。
...
実際に負帰還をかけて使うとき、オペアンプはv_in+とv_in-の差が小さくなる方向へv_outを動かす。

**結論**

入力電圧の差は、第1段で電流差と中間電圧へ変換され、第2段でさらに増幅されて出力電圧になる。
...
| MOSFET | 種類 | 接続または特徴 | 役割 |
|---|---|---|---|
| M1 | NMOS | ゲートをv_in-へ接続 | 差動入力の反転側を受け取る |
| M2 | NMOS | ゲートをv_in+へ接続 | 差動入力の非反転側を受け取る |
| M3 | PMOS | ゲートとドレインを接続 | 電流ミラーの基準側になる |
| M4 | PMOS | ゲートをM3と共通にする | M3の電流を写し、能動負荷になる |
| M5 | NMOS | M1、M2の共通ソース側に接続 | テール電流I_TAILを決める |
| M6 | NMOS | ゲートをv_1へ接続 | 第2段のソース接地増幅を行う |
| M7 | PMOS | V_DD側からv_outへ接続 | 第2段の電流源負荷になる |

役割を見分けるときは、次の接続に注目する。

- 2個のMOSFETのソースがつながっていれば、差動対の可能性がある
- ゲートとドレインがつながっていれば、電流ミラーの基準側の可能性がある
- 2個のMOSFETのゲートが共通なら、電流ミラーを構成している可能性がある
- ゲートへ信号が入り、ドレインから出力を取れば、ソース接地増幅回路の可能性がある
- 差動対の共通ソース側にある電流源は、テール電流源である

M5とM7のゲートには、一定のバイアス電圧が必要である。その電圧を作るために、実際のオペアンプでは別のMOSFETで構成したバイアス回路を追加する。

また、二段構成では高い利得が得られる一方、負帰還をかけたときに不安定になることがある。そのため、v_1とv_outの間に補償容量C_Cを接続することが多い。C_CはMOSFETではないが、安定した動作に重要な部品である。

**結論**

M1〜M5が第1段、M6とM7が第2段を作る。MOSFETの番号が変わっても、ゲート、ソース、ドレインの接続を見れば役割を判断できる。

---

## 第3章のまとめ

- M1とM2は、入力電圧の差を電流差へ変える
- M3とM4は、電流差を1つの中間電圧v_1へ変える
- M5は、第1段のテール電流を決める
- M6は、v_1を反転してさらに増幅する
- M7は、第2段の電流源負荷として働く
- 第1段の電流は第2段へ直接流れず、v_1がM6のゲートを動かす
- 全体利得は倍率では各段の利得を掛け、dBでは各段の利得を足す
- 各MOSFETを飽和領域で動作させるため、入力範囲と出力範囲には限界がある

二段構成CMOSオペアンプは、差動対、電流ミラー、電流源、ソース接地回路を順番につなぐことで、小さな入力電圧の差を大きな出力電圧へ変える。
