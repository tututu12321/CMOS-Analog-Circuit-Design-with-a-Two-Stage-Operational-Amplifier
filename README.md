...
<img width="630" height="567" alt="image" src="https://github.com/user-attachments/assets/af9b7449-208e-450c-982d-f28a105957b4" />
...
差動増幅回路は、`V_IN1`と`V_IN2`の電圧差だけを取り出して増幅し、2つの入力に共通して入ったノイズを抑える回路である。
...
`MN1`と`MN2`が入力MOSFETである。`MN3`は、差動対全体に流れる電流を作る。
...
負荷抵抗`R`は、MOSFETの電流変化を出力電圧の変化へ変換する。
...
差動増幅回路は、2つの入力電圧を比較し、その差を左右の出力電圧の差へ変換する回路である。
