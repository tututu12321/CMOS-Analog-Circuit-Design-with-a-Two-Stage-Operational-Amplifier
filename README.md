...
# 第4章　二段構成オペアンプの直流動作と低周波利得

## DC Operation and Low-Frequency Gain of a Two-Stage CMOS Operational Amplifier

この章では、二段構成オペアンプのMOSFETサイズを決め、直流動作と低周波利得を手計算する。

設計の目標は次のとおりである。

| 項目          | 目標                              |
| ----------- | ------------------------------- |
| 電源電圧        | `V_DD = +1.5 V`、`V_SS = -1.5 V` |
| 低周波開ループ利得   | `60 dB以上`                       |
| 出力の直流電圧     | 約`0 V`                          |
| MOSFETの動作領域 | `M1`から`M7`を飽和領域に保つ              |
...
低周波では補償容量`C_C`の影響が小さいため、この章の利得計算では`C_C`を無視する。
...
設計目標は、全体の低周波開ループ利得を`60 dB`以上にすることである。

電圧利得をdBから倍率へ戻すには、次の式を使う。

```text
abs(A_0) = 10^(A_0[dB] / 20)
```

`A_0`はオペアンプ全体の電圧利得である。電圧比をdBで表すときに`20`を使う。

`60 dB`を倍率へ変換する。

```text
abs(A_0) = 10^(60 / 20) = 1,000
```

この結果は、入力電圧差を少なくとも1,000倍にする必要があることを表している。

### 各段へ利得を割り振る

二段構成オペアンプでは、全体利得は第1段と第2段の利得を掛けた値になる。

```text
A_0 ≈ A_v1 × A_v2
```

`A_v1`は第1段の利得、`A_v2`は第2段の利得である。

全体で1,000倍にするため、2つの段へ同じ利得を割り振る場合を考える。

```text
sqrt(1,000) ≈ 31.6
```

この結果から、各段に約32倍の利得があれば、全体で約1,000倍になることが分かる。

実際にはプロセスばらつきや負荷の影響があるため、余裕を持たせて各段の目標を50倍とする。
...
各段を50倍にすれば、全体利得は次のようになる。

```text
abs(A_0) ≈ 50 × 50 = 2,500
```

2,500倍をdBへ変換すると約68 dBになるため、目標の60 dBに対して約8 dBの余裕が得られる。

---

## 4.2　利得を決めるパラメータ

MOSFET増幅回路の低周波利得は、相互コンダクタンスと出力抵抗の積で考えられる。

```text
abs(A_v) ≈ g_m × R_out
```

`g_m`は入力電圧の変化を電流変化へ変換する能力である。`R_out`は、その電流変化を出力電圧へ変換する抵抗である。

したがって、利得を大きくするには、`g_m`または`R_out`を大きくする必要がある。
...
`I_D`はMOSFETの直流ドレイン電流、`V_OV`はオーバードライブ電圧である。

この式は、電流を増やすか、`V_OV`を小さくすると`g_m`が大きくなることを表している。

### `r_o`を決めるもの

飽和領域のMOSFETの出力抵抗は、次の式で近似する。

```text
r_o ≈ 1 / (λ × I_D)
```

`λ`はチャネル長変調係数である。`λ`が小さいほど、MOSFETは理想的な電流源に近づき、`r_o`が大きくなる。

実際の設計では、チャネル長`L`を長くすると`λ`が小さくなり、利得を高くしやすい。

### 利得の設計思想

出力ノードに同程度の出力抵抗を持つ2個のMOSFETが接続される場合、合成出力抵抗はおよそ`r_o / 2`になる。

```text
R_out ≈ r_o / 2
```

これは、同じ値の抵抗を2本並列に接続すると、合成抵抗が半分になるためである。

この関係と`g_m`、`r_o`の式を組み合わせると、各段の利得はおおよそ次のようになる。

```text
abs(A_v) ≈ 1 / (λ × V_OV)
```

この式は、単純な長チャネルモデルでは、利得が主に`λ`と`V_OV`で決まることを表している。
...
したがって、`V_OV = 0.30 V`とすれば、目標とする約50倍の利得を得られる見込みがある。

| 設計で実現したいこと | 主に変更する値               |
| ---------- | --------------------- |
| 利得を大きくする   | `L`を長くする、`V_OV`を小さくする |
| 高速に動かす     | `I_D`を増やして`g_m`を大きくする |
| 消費電力を減らす   | `I_D`を減らす             |
| 電圧範囲を広げる   | `V_OV`を小さくする          |
...
第1段は小さな入力電圧差を検出する部分である。消費電力を抑えながら入力感度を確保するため、次の値を目標にする。
...
必要なドレイン電流を求めるため、`g_m`の式を`I_D`について変形する。

```text
I_D = g_m × V_OV / 2
```

この式は、必要な`g_m`と選択した`V_OV`から、MOSFETへ流す電流を逆算する式である。
...
したがって、`M1`と`M2`には、それぞれ`15 μA`を流す。

入力電圧が等しいとき、`M5`のテール電流は`M1`と`M2`へ半分ずつ流れる。そのため、`M5`の電流は次のようになる。

```text
I_D5 = I_D1 + I_D2 = 15 μA + 15 μA = 30 μA
```

この結果から、第1段全体へ流すテール電流を`30 μA`に決める。

### 第2段の電流

第2段は出力ノードを動かすため、第1段より大きな`g_m`が必要になる。
...
この結果から、`M6`へ`150 μA`を流す。

直流動作点では、`M6`が供給する電流と`M7`が引き込む電流を等しくする必要がある。

```text
I_D7 ≈ I_D6 = 150 μA
```

この関係によって、出力電圧が一方の電源側へ張り付かないようにする。

---

## 4.4　バイアス回路とMOSFETの大きさ
...
`V_TH`はしきい値電圧、`μC_ox`はMOSFETの電流の流しやすさ、`λ`はチャネル長変調の大きさを表す。

これらは自由に決める値ではなく、使用する製造プロセスやMOSFETモデルから得る。

### `M8`と抵抗`R`

`M8`は基準電流`I_REF`を作るMOSFETである。

この基準電流を、第1段のテール電流と同じ`30 μA`にする。

```text
I_REF = 30 μA
```

`M8`のゲート・ソース間電圧は、しきい値電圧とオーバードライブ電圧の和になる。

```text
V_GS8 = V_THn + V_OV8
```

この式は、MOSFETをオンさせるためのしきい値電圧に、実際に電流を流すための`V_OV`を加えることを表している。

`V_OV8 = 0.20 V`として計算する。

```text
V_GS8 = 0.5 V + 0.20 V = 0.70 V
```

`M8`のソースは`V_SS = -1.5 V`へ接続されている。そのため、ゲート電圧`V_B`は次のようになる。

```text
V_B = V_SS + V_GS8
```

この式は、ソース電圧から`V_GS8`だけ高い位置にゲート電圧があることを表している。

```text
V_B = -1.5 V + 0.70 V = -0.80 V
```

抵抗`R`には、`V_DD`と`V_B`の差の電圧が加わる。オームの法則から、必要な抵抗値は次のようになる。

```text
R = (V_DD - V_B) / I_REF
```

この式は、抵抗へ加わる電圧を、流したい基準電流で割って抵抗値を求めている。
...
したがって、抵抗`R`を約`76.7 kΩ`にすれば、基準電流は約`30 μA`になる。

### 電流ミラー比

`M5`には基準電流と同じ`30 μA`を流す。そのため、`M5`と`M8`の`W/L`を等しくする。

```text
(W / L)_5 = (W / L)_8
```

この関係によって、`M8`の電流が同じ大きさで`M5`へコピーされる。

`M7`には`150 μA`を流す。これは基準電流`30 μA`の5倍である。

```text
(W / L)_7 = 5 × (W / L)_8
```

この式は、`M7`の`W/L`を`M8`の5倍にすることで、約5倍の電流を流すことを表している。

### `W/L`を求める

飽和領域のMOSFET電流は、次の式で表される。

```text
I_D
=
(1 / 2) × μC_ox × (W / L) × V_OV^2
```

この式は、電流が`W/L`に比例し、`V_OV`の2乗に比例することを表している。
...
この式を使うと、決めた電流と`V_OV`を実現するために必要なMOSFETの大きさを求められる。
...
この結果から、`M1`と`M2`の`W/L`を約`1.67`にする。
...
`M6`はPMOSであり、さらに大きな電流を流すため、`M1`より大きな`W/L`が必要になる。

各MOSFETの計算結果をまとめる。

| MOSFET    |       電流 |   `V_OV` |  `W/L` |
| --------- | -------: | -------: | -----: |
| `M1`、`M2` |  `15 μA` | `0.30 V` | `1.67` |
| `M3`、`M4` |  `15 μA` | `0.30 V` | `3.33` |
| `M5`、`M8` |  `30 μA` | `0.20 V` |  `7.5` |
| `M6`      | `150 μA` | `0.30 V` | `33.3` |
| `M7`      | `150 μA` | `0.20 V` | `37.5` |
...
この式は、入力電圧差がなく、`M1`と`M2`へ同じ電流が流れている状態を表している。

`M1`のゲート・ソース間電圧は次のようになる。

```text
V_GS1 = V_THn + V_OV1 = 0.5 V + 0.30 V = 0.80 V
```

この結果は、`M1`へ`15 μA`を流すために、ゲートをソースより`0.80 V`高くする必要があることを表している。

入力電圧が`0 V`なので、共通ソース電圧`V_S`は次のようになる。

```text
V_S = v_inp - V_GS1 = 0 V - 0.80 V = -0.80 V
```

したがって、`M1`と`M2`の共通ソースは約`-0.80 V`になる。
...
この結果は、`M6`へ`150 μA`を流すために、ソースをゲートより`0.80 V`高くする必要があることを表している。

`M6`のソースは`V_DD = 1.5 V`へ接続されている。

```text
v_1 = V_DD - V_SG6 = 1.5 V - 0.80 V = 0.70 V
```

したがって、第1段は中間電圧`v_1`を約`0.70 V`にする必要がある。

### 出力電圧

出力ノードでは、`M6`が供給する電流と`M7`が引き込む電流がつり合う。

```text
I_D6 ≈ I_D7 ≈ 150 μA
```

この関係は、出力電圧が`V_DD`または`V_SS`へ張り付かないために必要である。

ここでは出力の直流動作点を中央の`0 V`に設定する。

```text
v_out ≈ 0 V
```

理想的な電流源だけでは`v_out`を一つの値に決められないため、実際には`M6`と`M7`の寸法を調整し、SPICEの直流解析で`v_out`を確認する。

### 飽和条件

NMOSが飽和領域にあるための条件は次のようになる。

```text
V_DS ≥ V_OV
```

この式は、ドレイン・ソース間に少なくとも`V_OV`以上の電圧が必要であることを表している。
...
PMOSでは電圧の向きが反対になるため、ソース・ドレイン間電圧`V_SD`を使う。

各MOSFETを確認する。

| MOSFET    |           実際の電圧 |           必要な電圧 | 判定 |
| --------- | --------------: | --------------: | -- |
| `M1`、`M2` | `V_DS = 1.50 V` | `V_OV = 0.30 V` | 飽和 |
| `M3`、`M4` | `V_SD = 0.80 V` | `V_OV = 0.30 V` | 飽和 |
| `M5`      | `V_DS = 0.70 V` | `V_OV = 0.20 V` | 飽和 |
| `M6`      | `V_SD = 1.50 V` | `V_OV = 0.30 V` | 飽和 |
| `M7`      | `V_DS = 1.50 V` | `V_OV = 0.20 V` | 飽和 |

すべてのMOSFETで実際の電圧が`V_OV`より大きいため、すべて飽和領域にある。

出力電圧の範囲は、下側の`M7`と上側の`M6`の飽和条件から決まる。
...
左側は`M7`が飽和領域に残るための下限、右側は`M6`が飽和領域に残るための上限を表している。
...
したがって、出力電圧が約`-1.30 V`から`1.20 V`の範囲にあれば、`M6`と`M7`を飽和領域に保てる。
...
この式は、`M1`のゲート電圧が変化したとき、ドレイン電流がどれだけ変化するかを表している。
...
この結果は、ゲート電圧が`1 mV`変化すると、ドレイン電流が約`0.10 μA`変化することを表している。
...
この式は、チャネル長変調によって生じるMOSFETの出力抵抗を表している。
...
`M4`にも同じ電流が流れるため、`r_o4`も約`1.0 MΩ`とする。

中間ノードからは、`r_o2`と`r_o4`が並列に見える。

```text
R_out1 = r_o2 || r_o4
```

この式は、中間ノードから見た第1段の合成出力抵抗を表している。
...
この式は、`g_m1`によって作られた電流変化が、`R_out1`によって電圧変化へ変換されることを表している。負号は、`v_inp`が上がると`v_1`が下がることを示している。
...
この式は、中間電圧`v_1`の変化によって、`M6`の電流がどれだけ変化するかを表している。
...
この結果は、`v_1`が`1 mV`変化すると、`M6`の電流が約`1 μA`変化することを表している。
...
`M7`にも同じ電流が流れるため、`r_o7`も約`100 kΩ`とする。

出力ノードから見た合成出力抵抗は次のようになる。

```text
R_out2 = r_o6 || r_o7
```

この式は、`M6`と`M7`の出力抵抗が並列に見えることを表している。
...
この式は、`M6`の電流変化を出力抵抗によって出力電圧へ変換することを表している。負号は、`v_1`が上がると`v_out`が下がることを示している。
...
したがって、第2段でも約50倍の利得を得られる。

---

## 4.7　全体の低周波開ループ利得

全体利得は、第1段と第2段の利得を掛けて求める。

```text
A_0 = A_v1 × A_v2
```

この式は、第1段で増幅された信号が、第2段でもう一度増幅されることを表している。
...
```text
A_0[dB] = 20 × log10(abs(A_0))
```

この式は、電圧利得の倍率を対数で表すための式である。
...
したがって、設計結果は目標の`60 dB`を満たしている。

| 確認項目        |        目標 |    計算結果 |
| ----------- | --------: | ------: |
| 低周波開ループ利得   | `60 dB以上` | `68 dB` |
| 出力の直流電圧     |    約`0 V` |  約`0 V` |
| MOSFETの動作領域 |     すべて飽和 |   すべて飽和 |

---

## 第4章の結論

この設計では、最初に必要な全体利得を決め、その利得を第1段と第2段へ分けた。

次に、各段に必要な`g_m`を決め、`g_m`と`V_OV`から電流を求めた。その電流を実現できるように`W/L`と抵抗`R`を決めた。

最後に、直流電圧と飽和領域を確認し、各段の利得を計算した。
