...
オペアンプの解析では、最初にDC解析を行い、その後にAC解析を行う。

それぞれの役割は、次のように考えるとわかりやすい。

```text
DC解析：信号がないときの状態を調べる
AC解析：周波数を変えたときの増幅の変化を調べる
```
...
| テール電流`I_TAIL`  |   `28 μA` |
| `M1`、`M2`の電流   |  約`14 μA` |
| 入力MOSFETの`g_m` | 約`100 μS` |
| 低周波利得          |  約`70 dB` |
| GBW            | 約`45 MHz` |
| 位相余裕           |    約`60°` |

これらは、実際の二段オペアンプを想定した数値例である。

---

### DC解析は「信号を入れる前」の確認

DC解析では、入力信号がないときの電圧と電流を求める。

例えば、2つの入力電圧が等しいとき、`M5`のテール電流`28 μA`は、`M1`と`M2`へほぼ半分ずつ流れる。
...
このように、信号がないときの基準となる状態を、動作点`Q`という。

---

### AC解析は「周波数を変えたとき」の確認

DC解析だけでは、オペアンプがどの周波数まで増幅できるか分からない。

そこで、動作点`Q`の周りに小さな交流信号を加えて、周波数ごとの利得と位相を調べる。

AC解析では、主に次の3つを確認する。

| 確認項目  |      数値例 | 意味            |
| ----- | -------: | ------------- |
| 低周波利得 |  `70 dB` | 低い周波数で約3,160倍 |
| GBW   | `45 MHz` | 利得が1倍になる周波数   |
| 位相余裕  |    `60°` | 発振しにくさの目安     |
...
ただし、周波数を高くすると利得は少しずつ低下する。AC解析では、この変化をボード線図で確認する。

---

### 小さな変化だけを考える

MOSFETの電圧と電流の関係は、本来は曲線である。

しかし、動作点の近くだけを見れば、ほぼ直線として扱うことができる。
...
つまり、AC解析では、`14 μA`という直流電流を中心にした小さな変化を調べている。

---

### 大文字と小文字の違い

直流値と小さな変化は、文字の大きさで区別する。

| 表記     |       数値例 | 意味           |
| ------ | --------: | ------------ |
| `V_GS` | `0.700 V` | DC解析で求めた直流電圧 |
| `v_gs` |    `1 mV` | 直流電圧からの小さな変化 |
| `I_D`  | `14.0 μA` | DC解析で求めた直流電流 |
| `i_d`  | `0.10 μA` | 直流電流からの小さな変化 |

実際の値は、直流値と小信号を足したものになる。
...
というMOSFETの基本動作を表している。

`v_ds/r_o`は、ドレイン電圧の変化によって生じる電流変化を表す補正項である。

---

### SPICEでAC振幅を1 Vにする理由

SPICEのAC解析では、入力のAC振幅を`1 V`に設定することが多い。

しかし、実際に振幅`1 V`の大きな正弦波をMOSFETへ加えているわけではない。

SPICEは回路を動作点`Q`の周りで直線化し、
...
入力を`1 V`にしておくと、出力の値をそのまま電圧利得として読み取れるため便利である。

**結論**

DC解析では、信号がないときの電圧と電流を求め、MOSFETが正しく動作しているか確認する。

AC解析では、その動作点の周りに小さな信号を加え、周波数ごとの利得と位相を調べる。
...
交流回路では、信号の「大きさ」だけでなく「位相のずれ」も考える必要がある。
...
複素数を使うと、この2つをまとめて計算できる。

```text
e^(iθ) = cosθ + i sinθ
```

交流回路では、正弦波を`e^(iωt)`として表すことで、微分や積分を簡単に扱える。

---

### 複素周波数`s`

ラプラス変換では、複素周波数`s`を使う。
...
通常のAC解析では、一定の大きさで振動する正弦波を考える。そのため、
...
抵抗の値は、周波数が変わっても一定である。

コンデンサは、周波数が高くなるほど電流を通しやすくなる。
...
例えば、抵抗`R`とコンデンサ`C`を使ったRCローパスフィルタでは、
...
* `15.9 kHz`より低い周波数：信号を通しやすい
* `15.9 kHz`より高い周波数：信号を小さくする

このように、伝達関数を使うと、周波数による利得と位相の変化を計算できる。

---

### オペアンプの伝達関数

二段オペアンプの伝達関数は、おおよそ次のように表せる。
...
この極や零点の位置を調べることで、オペアンプの速度と安定性を確認できる。

**結論**

複素数は、交流信号の大きさと位相をまとめて表すために使う。

ラプラス変換は、`R`・`C`・`L`を含む回路の微分方程式を計算しやすくする。

伝達関数は、入力から出力までの利得と位相が、周波数によってどのように変わるかを表す。
